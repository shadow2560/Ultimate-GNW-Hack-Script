	...
	if [ $4 -ge 3 ]; then
		if [ $size -eq 4 ]; then #triple boot 4MB
			...
			if [ "$2" == "zelda" ]; then
				...
				make -j$6 GNW_TARGET=$2 INTFLASH_ADDRESS=0x08020000 EXTFLASH_SIZE_MB=$size EXTFLASH_OFFSET=6291456  ADAPTER=$1 BIG_BANK=0 flash
			else
				((size=size-3))
				((fs_size=((size*$17)/100)*1024*1024))
				echo make -j$6 GNW_TARGET=$2 INTFLASH_ADDRESS=0x08020000 EXTFLASH_SIZE_MB=$size EXTFLASH_OFFSET=3145728  ADAPTER=$1 BIG_BANK=0 flash
				make -j$6 GNW_TARGET=$2 INTFLASH_ADDRESS=0x08020000 EXTFLASH_SIZE_MB=$size EXTFLASH_OFFSET=3145728  ADAPTER=$1 BIG_BANK=0 flash
			fi
		fi
	elif [ $4 -eq 1 ]; then
		if [ $size -eq 4 ]; then #dual boot with CFW 4MB
			...
			if [ "$2" == "zelda" ]; then
				...
				((size=size-1))
				((fs_size=((size*$17)/100)*1024*1024))
				echo make -j$6 GNW_TARGET=$2 INTFLASH_BANK=2 EXTFLASH_SIZE_MB=$size EXTFLASH_OFFSET=1048576 ADAPTER=$1 STATE_SAVING=$7 CODEPAGE=$8 UICODEPAGE=$8 COVERFLOW=$9 ENABLE_SCREENSHOT=${10} CHEAT_CODES=${11} SHARED_HIBERNATE_SAVESTATE=${12} DISABLE_SPLASH_SCREEN=${13} FORCE_NOFRENDO=${14} flash
				make -j$6 GNW_TARGET=$2 INTFLASH_BANK=2 EXTFLASH_SIZE_MB=$size EXTFLASH_OFFSET=1048576 ADAPTER=$1 STATE_SAVING=$7 CODEPAGE=$8 UICODEPAGE=$8 COVERFLOW=$9 ENABLE_SCREENSHOT=${10} CHEAT_CODES=${11} SHARED_HIBERNATE_SAVESTATE=${12} DISABLE_SPLASH_SCREEN=${13} FORCE_NOFRENDO=${14} flash
			fi
		fi
	elif [ $4 -eq 2 ]; then
