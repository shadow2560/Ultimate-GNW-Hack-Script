...
if [ ! -z "$run" ]; then
	...
	if [ $4 -eq 1 ]; then
		if [ $3 -eq 4 ]; then #dual boot with CFW on 4MByte flash
			echo make -j$6 INTFLASH_BANK=2 EXTFLASH_SIZE=2097152 EXTFLASH_OFFSET=868352 ADAPTER=$1 GNW_TARGET=$2 DIALOGUES_LANGUAGE=$7 ENABLE_SAVESTATE=$8 flash
			make -j$6 INTFLASH_BANK=2 EXTFLASH_SIZE=2097152 EXTFLASH_OFFSET=868352 ADAPTER=$1 GNW_TARGET=$2 DIALOGUES_LANGUAGE=$7 ENABLE_SAVESTATE=$8 flash
		else
