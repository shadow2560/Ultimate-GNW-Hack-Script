	...
	if [ $4 -eq 1 ]; then
		if [ $3 -eq 4 ]; then #dual boot with CFW on 4MByte flash
			...
			if [ "$2" == "zelda" ]; then
				...
				((size=size-1))
				echo make -j$6 INTFLASH_BANK=2 EXTFLASH_SIZE=2097152 EXTFLASH_OFFSET=1048576 GNW_TARGET=$2 flash ADAPTER=$1 LARGE_FLASH=%LARGE_FLASH DIALOGUES_LANGUAGE=$7 ENABLE_SAVESTATE=$8 flash
				make -j$6 INTFLASH_BANK=2 EXTFLASH_SIZE=2097152 EXTFLASH_OFFSET=1048576 GNW_TARGET=$2 flash ADAPTER=$1 LARGE_FLASH=%LARGE_FLASH DIALOGUES_LANGUAGE=$7 ENABLE_SAVESTATE=$8 flash
			fi
		fi
	elif [ $4 -eq 2 ]; then
